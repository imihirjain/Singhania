...
function GreyTable() {
  ...
  const checkAdminRole = async () => {
    try {
      const response = await axios.get(
        "https://singhania-inventory.onrender.com/api/login",
        {
  ...
  const fetchSubmittedData = async () => {
    try {
      const response = await axios.get(
        "https://singhania-inventory.onrender.com/api/sales/data/all/reject",
        {
          withCredentials: true,
        }
      );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `https://singhania-inventory.onrender.com/api/delete/${id}`,
        {
          withCredentials: true,
        }
      );
  ...
  const handleUpdate = async (id, data) => {
    try {
      const response = await axios.put(
        `https://singhania-inventory.onrender.com/api/update/${id}`,
        data,
        {
          withCredentials: true,
        }
      ...
      <div className="w-full mt-6 bg-white border-nav border-2 rounded-lg">
        <div className="lg:flex ml-4 mt-4 justify-between items-center ">
          <div className="text-title font-bold">
            Grey Stock OUT Table <br /> Total:{filteredData.length}
          </div>
                ...
                {currentItems.map((dataItem, index) => (
                  ...
                  >
                    ...
                    {isAdmin && (
                      <td>
                        <button
                          onClick={() => handleUpdate(dataItem.id, dataItem)}
