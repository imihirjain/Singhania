...
function DispatchTable() {
  ...
  const handleEditConfirm = async () => {
    try {
      const updatedEntry = {
        ...
        karigarName: selectedEntry.karigarName,
      };
  ...
  const groupEntries = (data) => {
    const groupedData = {};
    data.forEach((item) => {
      const key = `${item.lotNumber}-${item.party}-${item.quality}-${item.shade}-${item.process}-${item.status}`;
      if (!groupedData[key]) {
        groupedData[key] = [];
      }
  ...
  const handleSearch = () => {
    ...
    const filteredData = submittedData.filter(
      (item) =>
        (item.partyName && item.partyName.toLowerCase().includes(query)) ||
        (item.qualityChallanNumber &&
          item.qualityChallanNumber.toLowerCase().includes(query))
    );
    return filteredData;
  };
          ...
          <div className="flex flex-col items-center mt-8">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md border-2">
              ...
              <form className="flex flex-col items-center">
                ...
                <div className="border-2 w-full sm:w-[500px] h-full rounded-lg mt-3 shadow-sm shadow-darkgray">
                  <label className="block text-lg font-login ml-4 mt-4 text-gray-700">
                    Karigar Name<span className="text-red-600 ml-1">*</span>
                    <input
                      className="block w-[300px] mt-2 p-2 border-2 rounded mb-4"
                      name="karigarName"
                      value={selectedEntry?.karigarName || ""}
